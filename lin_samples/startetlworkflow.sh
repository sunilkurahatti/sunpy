...
$paramfile = $ENV{'PMRootDir'} . "/paramfiles/" . $workflow . "_" . $po_id . "_" . $batch_id . ".par";
$param_archive_loc=$ENV{'NAS'} . "/infodel/archive/monthly/" . $workflow . "_" . $po_id . "_" . $batch_id . ".par";
$runinstance = $po_id . "_" . $batch_id;
...
$paramfile = $ENV{'PMRootDir'} . "/paramfiles/" . $workflow . "_" . $firm_id . "_" . $batch_id . ".par";
$param_archive_loc=$ENV{'NAS'} . "/infodel/archive/monthly/" . $workflow . "_" . $firm_id . "_" . $batch_id . ".par";
$runinstance = $firm_id . "_" . $batch_id;
...
system("$extprocdir/paramfilecreation.sh parentpid=$$") == 0 || die "Error : Cannot call or error in the parameterfilecreation.sh script \n";
...
#
my $pmcmd;my $rc;my $acrc;
if ( $startfrom eq "N" )
{
$pmcmd=`pmcmd startworkflow -sv $ENV{'INT_SVC'} -d $ENV{'DOMAIN_NAME'} -u wf_runner -p wfrunner -f "$folder" -paramfile "$paramfile" -wait -rin "$runinstance" "$workflow"`;
$rc = $? >> 8;
$acrc = $?; 
...
{
$pmcmd=`pmcmd startworkflow -sv $ENV{'INT_SVC'} -d $ENV{'DOMAIN_NAME'} -u wf_runner -p wfrunner -f "$folder" -startfrom "$startfrom" -paramfile "$paramfile" -wait -rin "$runinstance" "$workflow"`;
$rc = $? >> 8; 
$acrc = $?;
move($paramfile,$param_archive_loc) || die "Error moving the generated parameter file to archive location $param_archive_loc \n";
...
# Adding Condition for Workflow Hang and returning successful message - 

if ($rc = 0 and $acrc = 9) {
    print "ERROR: The connection to the PowerCenter Server timed out while sending the request:\n\n";
    print $pmcmd;
    # Exit with whatever code pmcmd returned 
    exit $acrc; 
} 
